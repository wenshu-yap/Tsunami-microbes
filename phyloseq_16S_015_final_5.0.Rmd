...
qual_col <- c("#457291", "#CF6D2D", "#97B384", "#DEC447", "#8B4452", "#1E4650", "#F2A86A", "#7DB8DC", "#DDDDDD")
...
### 4.5.4 Plotting PCoA
Plotting PCoA for all the samples
```{r}
  ...
  theme_bw() +
    theme(plot.title = element_text(size = 12, hjust = 0.5, face = 'bold'),
          ...
          legend.box = "vertical",
            panel.background = element_blank(),
            panel.grid.minor = element_blank()) +
    labs(title = str_c(ps_loc_list$title[[i]], "- dbRDA - bray_curtis"))
...
save plots as grid
```{r}
#grid_fig_dbrda_loc<- ggarrange(array_dbrda_loc[[1]],array_dbrda_loc[[2]],
#                                array_dbrda_loc[[3]],array_dbrda_loc[[4]],
#                          labels = c("A","B", "C","D"),
#                          align = "hv", 
#                          ncol = 2, nrow =2,
#                          #common.legend = TRUE,
...
#    grid_fig_dbrda_loc
  #ggsave("./phyloseq/fig_final/fig_dbrda_loc_combined_80.pdf", grid_fig_dbrda_loc, 
  #        height = 20, width = 20, dpi = 300)
```
...
#### 4.7.8.1 Load required libraries
```{r}
...
#### 4.7.8.2 plotting for Cuddalore
...
# pdf("./phyloseq/fig_final/deseq_india_revised_label3.pdf", 
...
#### 4.7.8.3 plotting for Phra Thong
```{r, fig.height = 5, fig.width=8}
# filter the data
...
t.cluster_pt <- hclust(t.distance_pt, method = "ward") 
 ...
 pdf("./phyloseq/fig_final/deseq_pt_col2_revised_label2.pdf", 
    width=3, height=6, pointsize = 8)
